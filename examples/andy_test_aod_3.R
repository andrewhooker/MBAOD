...
nid=20
nsteps=4
name=paste("5_2_nsteps_",nsteps,"_nid_",nid,"_missp_",paste(missp[[1]], collapse="-"),sep="")

models = list(modfullest="run2fE.mod", modfullsim="run2fS.mod",
                       modredest="run3rE.mod", modredsim="run2rS.mod")

## initial design of first group
current <- list(list(ngroups       = 1,
                     dose          = list(1000),   
                     nid           = 50, 
                     ...
                     ...)
{
  ...
  if(file.exists(paste("result_",name,".Rdata",sep=""))){
    next()
  }
  
  ## remove old folder
  if(file.exists(name) && remove.old.folder){
    system(paste('rm -r', name, sep=" "))
  } else {
    next("Folder already exists")
  }
