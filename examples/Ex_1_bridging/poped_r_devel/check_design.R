#' Check the design variables for inconsistencies in size, adjust if necessary.
#' 
#' \code{check_design_variables} Checks the sizes of design varibales and 
  ...
  model_switch=NULL) ## -- Vector defining which response a certain sampling time belongs to, defaults to one for all elements of xt --
{
  ...
  if(!is.null(a)){
    ...
    if(size(a,1)==1 && m!=1) a <- matrix(rep(a,m),ncol=length(a),nrow=m,byrow=T,dimnames=list(paste("grp_",1:m,sep=""),names(a)))
  ...
  if(!is.null(x)){
    ...
    if(size(x,1)==1 && m!=1) x <- matrix(rep(x,m),ncol=length(x),nrow=m,byrow=T,dimnames=list(paste("grp_",1:m,sep=""),names(x)))
