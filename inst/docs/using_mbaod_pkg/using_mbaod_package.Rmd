...
## Optimal design (no adaptation) with the wrong initial parameters

We can also compare what would happen if we performed the optimization only once, directly after the adult data comes in.  First we assume that we have the wrong guesses for the  maturation model:
...
```{r load_example_1_e_rep_100,echo=FALSE}
load("Example_1_e_rep_100/results_all.Rdata")
```
...
```{r, fig.cap="Parameter estimates for all 100 iterations. Zooming in on the y-axis"}
## zoomig in a bit
plot4 <- plot_parameter_estimates(results_all,unlist(parameters_true_sd))
plot4a <- plot4 + coord_cartesian(xlim = NULL, ylim= c(-50,50))
plot4a
```

There is a clear bias in parameter estimates for THETA(4) and THETA(5) ($WT_{50}$ and $\gamma$).