#' Create a design and design space for optimization.
#' 
#' \code{create_design_space} takes a design and arguments for a design space and 
  ...
  grouped_x=NULL)
{
  ...
  with(design,{
    ...
    if(size(maxxt,1)==1 && m!=1) xt <- matrix(rep(maxxt,m),ncol=length(maxxt),nrow=m,byrow=T)
    if(!is.matrix(maxxt)) maxxt <- rbind(maxxt)
    if(size(maxxt,1)!=m){
      stop("The number of rows in maxxt (", size(maxxt,1), 
    ...
    if(size(minxt,1)==1 && m!=1) xt <- matrix(rep(minxt,m),ncol=length(minxt),nrow=m,byrow=T)
